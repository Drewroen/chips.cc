...
export class GameService
{
    static tick(game: Game)
    {
        if (game.gameStatus === Constants.GAME_STATUS_PLAYING) {
            game.timer--;
            if (game.timer <= 0) this.endGameplay(game);
            ...
            game.players?.forEach((player) => {
              ...
              if (playerCoords) {
                ...
                if (
                  ...
                  game.isForceField(playerTerrain.value) &&
                  ...
                  playerCoords &&
                  player.slipCooldown <= 0 &&
                  game.isRandomForceField(playerTerrain.value) &&
                  !player.inventory.forceBoots
                ) {
                  const forceTile = playerTerrain as ForceTile;
                  ...
                  playerCoords &&
                  player.slipCooldown <= 0 &&
                  game.isIce(playerTerrain.value) &&
                  ...
                  if (
                    ...
                    if (terrainTile.value === Constants.TERRAIN_TELEPORT) {
                      ...
                      if (
                        ...
                        0 &&
                      !game.isForceField(terrainTile.value) &&
                      !game.isIce(terrainTile.value) &&
                      !game.isRandomForceField(terrainTile.value) &&
            ...
            game.timer--;
            if (game.timer <= 0) this.startGamePlay(game);
