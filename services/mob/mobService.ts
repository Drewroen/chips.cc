...
import { MobTile, BallTile, FireballTile, BlobTile, BlockTile, BowlingBallTile, BugTile, GliderTile, ParemeciumTile, TankTile, TeethTile, WalkerTile } from './../../objects/mobTile';
...
export class MobService {
    static move(game: Game, mobTile: MobTile): void {
        ...
        if (coords) {
            ...
            for (const directionAttempt of preferredDirections) {
                if (!game.gameMap.getTerrainTile(x, y).getBlockedMobDirections(game, mobTile.id).includes(directionAttempt)) {
                    let newX = x;
                    let newY = y;
                    switch (directionAttempt) {
                        case Constants.DIRECTION_UP: newY = (y - 1 + Constants.MAP_SIZE) % Constants.MAP_SIZE; break;
                        case Constants.DIRECTION_DOWN: newY = (y + 1 + Constants.MAP_SIZE) % Constants.MAP_SIZE; break;
                        case Constants.DIRECTION_LEFT: newX = (x - 1 + Constants.MAP_SIZE) % Constants.MAP_SIZE; break;
                        case Constants.DIRECTION_RIGHT: newX = (x + 1 + Constants.MAP_SIZE) % Constants.MAP_SIZE; break;
                        default: break;
                    }
                    ...
                    if (this.mobCanMoveToCoordinates(game, mobTile, newX, newY, directionAttempt)) {
                        mobTile.direction = directionAttempt;
                        this.setSpriteBasedOnDirection(mobTile);
                        game.gameMap.getTerrainTile(newX, newY).interactionFromMob(game, mob.id, newX, newY);
                        if (game.findMob(mob.id).alive)
                            game.gameMap.getObjectTile(newX, newY)?.interactionFromMob(game, mob.id, newX, newY);
                        if (game.findMob(mob.id).alive) {
                            if (mobTile instanceof BowlingBallTile) {
                                ...
                                this.interactionFromMob(game, mob.id, game.gameMap.getMobTile(newX, newY));
                        }
                        if (mob.alive) {
        ...
        if (mobTile instanceof BlockTile)
            mobTile.direction = null;
    }

    private static getPreferredDirections(game: Game, mobTile: MobTile): number[] {
        const coords = game.findMobTileCoordinates(mobTile.id);
        ...
        if (game.isForceField(game.gameMap.getTerrainTile(coords[0], coords[1]).value))
            ...
            return [direction];
        else if (game.isIce(game.gameMap.getTerrainTile(coords[0], coords[1]).value)) {
        ...
        else
        {
            ...
            if (mobTile instanceof BugTile) return [ (direction + 3) % 4, direction, (direction + 1) % 4, (direction + 2) % 4 ];
            else if (mobTile instanceof BallTile) return [direction, (direction + 2) % 4];
            else if (mobTile instanceof FireballTile) return [ direction, (direction + 1) % 4, (direction + 3) % 4, (direction + 2) % 4 ];
            else if (mobTile instanceof BlobTile) return [ direction, (direction + 1) % 4, (direction + 2) % 4, (direction + 3) % 4 ].sort(() => Math.random() - 0.5);
            else if (mobTile instanceof BowlingBallTile) return [ direction ];
            else if (mobTile instanceof GliderTile) return [ direction, (direction + 3) % 4, (direction + 1) % 4, (direction + 2) % 4 ];
            else if (mobTile instanceof WalkerTile) return [direction].concat([ (direction + 1) % 4, (direction + 3) % 4, (direction + 2) % 4 ].sort(() => Math.random() - 0.5));
            else if (mobTile instanceof ParemeciumTile) return [ (direction + 1) % 4, direction, (direction + 3) % 4, (direction + 2) % 4 ];
                ...
                for (let i = 0; i < Constants.MAP_SIZE; i++)
                  ...
                  {
                    if (game.gameMap.getMobTile(i, j) instanceof PlayerTile && game.gameMap.getMobTile(i, j).id !== game.findMob(mobTile.id).ownerId)
                    {
    ...
    private static mobCanMoveToCoordinates(game: Game, mobTile: MobTile, x: number, y: number, direction: number) {
        if (game.gameMap.getTerrainTile(x, y).solid(game, mobTile.id, direction) ||
    ...
    public static setSpriteBasedOnDirection(mobTile: MobTile) {
        if (mobTile instanceof PlayerTile)
        {
            switch(mobTile.direction) {
                case (Constants.DIRECTION_UP): mobTile.value = Constants.MOB_PLAYER_UP; break;
                case (Constants.DIRECTION_LEFT): mobTile.value = Constants.MOB_PLAYER_LEFT; break;
                case (Constants.DIRECTION_DOWN): mobTile.value = Constants.MOB_PLAYER_DOWN; break;
                case (Constants.DIRECTION_RIGHT): mobTile.value = Constants.MOB_PLAYER_RIGHT; break;
              }
        }
        else if (mobTile instanceof BallTile)
            mobTile.value = Constants.MOB_BALL;
        else if (mobTile instanceof FireballTile)
            mobTile.value = Constants.MOB_FIREBALL;
        else if (mobTile instanceof BlobTile)
            mobTile.value = Constants.MOB_BLOB;
        else if (mobTile instanceof BlockTile)
            mobTile.value = Constants.MOB_BLOCK;
        else if (mobTile instanceof BowlingBallTile)
            mobTile.value = Constants.MOB_BOWLING_BALL;
        else if (mobTile instanceof BugTile) {
            switch (mobTile.direction) {
                case (Constants.DIRECTION_UP): mobTile.value = Constants.MOB_BUG_UP; break;
                case (Constants.DIRECTION_LEFT): mobTile.value = Constants.MOB_BUG_LEFT; break;
                case (Constants.DIRECTION_DOWN): mobTile.value = Constants.MOB_BUG_DOWN; break;
                case (Constants.DIRECTION_RIGHT): mobTile.value = Constants.MOB_BUG_RIGHT; break;
            }
        }
        else if (mobTile instanceof GliderTile) {
            switch (mobTile.direction) {
                case (Constants.DIRECTION_UP): mobTile.value = Constants.MOB_GLIDER_UP; break;
                case (Constants.DIRECTION_LEFT): mobTile.value = Constants.MOB_GLIDER_LEFT; break;
                case (Constants.DIRECTION_DOWN): mobTile.value = Constants.MOB_GLIDER_DOWN; break;
                case (Constants.DIRECTION_RIGHT): mobTile.value = Constants.MOB_GLIDER_RIGHT; break;
            }
        }
        else if (mobTile instanceof ParemeciumTile) {
            switch (mobTile.direction) {
                case (Constants.DIRECTION_UP): mobTile.value = Constants.MOB_PAREMECIUM_UP_DOWN; break;
                case (Constants.DIRECTION_LEFT): mobTile.value = Constants.MOB_PAREMECIUM_LEFT_RIGHT; break;
                case (Constants.DIRECTION_DOWN): mobTile.value = Constants.MOB_PAREMECIUM_UP_DOWN; break;
                case (Constants.DIRECTION_RIGHT): mobTile.value = Constants.MOB_PAREMECIUM_LEFT_RIGHT; break;
            }
        }
            ...
            switch (mobTile.direction) {
                case (Constants.DIRECTION_UP): mobTile.value = Constants.MOB_TANK_UP; break;
                case (Constants.DIRECTION_LEFT): mobTile.value = Constants.MOB_TANK_LEFT; break;
                case (Constants.DIRECTION_DOWN): mobTile.value = Constants.MOB_TANK_DOWN; break;
                case (Constants.DIRECTION_RIGHT): mobTile.value = Constants.MOB_TANK_RIGHT; break;
            }
        }
        else if (mobTile instanceof TeethTile) {
            switch (mobTile.direction) {
                case (Constants.DIRECTION_UP): mobTile.value = Constants.MOB_TEETH_UP; break;
                case (Constants.DIRECTION_LEFT): mobTile.value = Constants.MOB_TEETH_LEFT; break;
                case (Constants.DIRECTION_DOWN): mobTile.value = Constants.MOB_TEETH_DOWN; break;
                case (Constants.DIRECTION_RIGHT): mobTile.value = Constants.MOB_TEETH_RIGHT; break;
            }
        }
        else if (mobTile instanceof WalkerTile) {
            switch (mobTile.direction) {
                case (Constants.DIRECTION_UP): mobTile.value = Constants.MOB_WALKER_UP_DOWN; break;
                case (Constants.DIRECTION_LEFT): mobTile.value = Constants.MOB_WALKER_LEFT_RIGHT; break;
                case (Constants.DIRECTION_DOWN): mobTile.value = Constants.MOB_WALKER_UP_DOWN; break;
                case (Constants.DIRECTION_RIGHT): mobTile.value = Constants.MOB_WALKER_LEFT_RIGHT; break;
            }
        }
    }
    ...
    public static kill(game: Game, mobTile: MobTile) {
        ...
        if (mobTile instanceof PlayerTile)
        {
            game.players.map(player => {if(player.id === mobTile.id) (player.kill())});
            const coords: number[] = game.findPlayerCoordinates(mobTile.id);
            game.gameMap.setMobTile(coords[0], coords[1], null);
        }
        else
        {
            game.mobs.map(mob => { if (mob.id === mobTile.id) (mob.kill()) });
            const coords = game.findMobTileCoordinates(mobTile.id);
            game.gameMap.setMobTile(coords[0], coords[1], null);
        }
    }

    public static interactionFromPlayer(game: Game, interactingId: string, mobTile: MobTile) {
        if (mobTile == null || mobTile instanceof PlayerTile)
            return;
        if (!(mobTile instanceof BlockTile))
    ...
    public static interactionFromMob(game: Game, interactingId: string, mobTile: MobTile) {
        if (mobTile instanceof PlayerTile)
        {
            game.kill(mobTile.id);
    ...
    public static solid(game: Game, interactingId: string, interactingDirection: number, mobTile: MobTile) {
        if (mobTile == null)
            return false;
        else if (mobTile instanceof PlayerTile)
        {
            ...
            if (mobTile instanceof BlockTile) {
                ...
                if (coords && newCoords) {
                    if (coords[0] === newCoords[0] && coords[1] === newCoords[1]) {
                        ...
                        if (mobTile.lastHitTime <= 0 || mobTile.lastHitId !== interactingId) {
                            mobTile.health--;
                            ...
                            switch (mobTile.health) {
                                ...
                                case 0:
                                    MobService.kill(game, game.gameMap.getMobTile(coords[0], coords[1]));
                ...
                return game.findMob(mobTile.id).ownerId === interactingId;
        }
        else if (game.findMobTile(interactingId) instanceof BowlingBallTile && !(mobTile instanceof BowlingBallTile))
            return false;
        else if (game.findMob(interactingId) && mobTile instanceof BowlingBallTile)
            return false;
        else if (game.findMob(interactingId))
            return true;
        return true;
    }
}
