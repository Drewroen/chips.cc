...
export class EloService {
  ...
  async calculateEloValues(game: GameRoom): Promise<EloResult[]> {
    const accountsToScore = game.game.players.map(player => player.name).filter(name => name !== 'Chip');
    ...
    const accountBatchGetResponse = await Dynamo.batchGet(accountParams, this.dynamoDb);

    const eloResults: EloResult[] = accountBatchGetResponse.Responses.ChipsMMOAccounts.map(account => {
      return new EloResult(account.Username as string, account.ELO as number, account.ELO as number);
    })
  ...
  async updateEloValues(eloResults: EloResult[]) {
    eloResults.forEach(async result => {
      const accountResponse = await Dynamo.get(this.createEloRequest(result.id), this.dynamoDb);
      const updateEloRequest = this.createUpdateEloRequest(accountResponse, result.newElo);
